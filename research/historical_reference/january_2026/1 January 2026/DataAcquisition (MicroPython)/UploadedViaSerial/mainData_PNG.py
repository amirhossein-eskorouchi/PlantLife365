...
# COMPRESSION QUALITY (0-100, higher = better quality, larger files)
PNG_QUALITY = 85  # Adjust: 90=high quality, 70=smaller files, 100=lossless PNG

PORT = '/dev/ttyUSB0'  # Change to COM3 on Windows
DIR = f'{date.today()}_Data_PNG_{PNG_QUALITY}'
...
print(f"Listening for data... (Compression quality: {PNG_QUALITY}/100)")
        ...
        # End of image
            ...
            try:
                ...
                if len(img_data) == expected_size:
                    ...
                    if img is not None:
                        filename = f"img_{last_counter}.png"
                        image_path = os.path.join(DIR, filename)
                        
                        # Save with configurable compression quality
                        params = [cv2.IMWRITE_PNG_COMPRESSION, (9 - PNG_QUALITY // 12)]
                        success = cv2.imwrite(image_path, img, params)
                        
                        if success:
                            print(f"✓ SAVED PNG: {image_path} (quality {PNG_QUALITY}/100)")
                        else:
                            print(f"✗ Failed to save PNG: {image_path}")
                    else:
