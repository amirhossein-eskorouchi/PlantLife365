...
# JPEG COMPRESSION QUALITY (0-100, higher = better quality, larger files)
JPEG_QUALITY = 50  # Adjust: 95=excellent, 85=good, 75=smaller files

PORT = '/dev/ttyUSB0'  # Change to COM3 on Windows
DIR = f'{date.today()}_Data_JPEG_{JPEG_QUALITY}'
...
print(f"Listening for data... (JPEG quality: {JPEG_QUALITY}/100)")
        ...
        # End of image
            ...
            try:
                ...
                if len(img_data) == expected_size:
                    ...
                    if img is not None:
                        filename = f"img_{last_counter}.jpg"
                        ...
                        if success:
                            print(f"✓ SAVED JPEG: {image_path} (quality {JPEG_QUALITY}/100)")
                        else:
                            print(f"✗ Failed to save JPEG: {image_path}")
                    else:
